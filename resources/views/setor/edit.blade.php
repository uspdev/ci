  ...
  <div class="row">
        ...
        <div class="card-body">
          <form action="{{ route('setor.update', $setor->id) }}" method="POST">
            ...
            <div class="form-group">
              ...
              <input type="text" id="name" name="name" class="form-control" value="{{ $setor->name }}"
                required>
            </div>

            <div class="form-group">
              <label for="description">Descrição</label>
              <textarea id="description" name="description" class="form-control" rows="3">{{ $setor->description }}</textarea>
            </div>

            <div class="form-group">
              <button type="submit" class="btn btn-primary" disabled>Salvar Alterações</button>
              <a href="{{ route('setor.index') }}" class="btn btn-secondary">Cancelar</a>
            </div>
          </form>
        </div>
      </div>
    ...
    <div class="col-md-4">
      <div class="card h-100">
        <form method="post" id="form-{{ $setor->id }}"
          action="{{ route('setor.editarResponsavel', ['id' => $setor->id]) }}">
          @csrf
          @method('put')
          ...
          <div class="card-header h5 py-2">
            ...
            <div class="small text-secondary">Pessoas autorizadas a gerenciar o setor</div>
          </div>

          <div class="card-body py-1">
            @foreach ($setor->users() as $admin)
